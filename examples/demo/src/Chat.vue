...
<style scoped>
.page { height: 100vh; display: flex; flex-direction: column; background: var(--app-page-bg); color: var(--base-content-color); overflow: hidden; }
.global-header { height: 52px; border-bottom: 1px solid var(--app-border-soft); display: flex; align-items: center; justify-content: space-between; padding: 0 16px; background: var(--app-header-bg); backdrop-filter: blur(8px); }
.brand { font-weight: 700; letter-spacing: .08em; text-transform: uppercase; background: var(--app-brand-gradient); -webkit-background-clip: text; background-clip: text; color: transparent; }
...
.nav-link { color: var(--app-accent-text); text-decoration: none; font-size: 12px; border: 1px solid var(--app-accent-border); background: var(--app-accent-bg); padding: 6px 10px; border-radius: 8px; }
.nav-link:hover { background: var(--app-accent-bg-hover); }
...
.sidebar { display: flex; flex-direction: column; gap: 12px; padding: 12px; border-radius: 16px; border: 1px solid var(--app-border-soft); background: var(--app-surface-1); box-shadow: 0 8px 20px var(--app-shadow-soft); position: relative; font-size: 12px; }
.panel { border: 1px solid var(--app-border-soft); border-radius: 14px; padding: 14px; background: var(--app-surface-1); }
...
.question-item { font-family: 'JetBrains Mono', monospace; color: var(--app-success); font-size: 12px; line-height: 1.55; }
.question-item .prompt, .question-item .question-text { color: var(--app-success); }
...
.terminal-wait { color: var(--app-muted-text); }
...
.hero-brand { margin: 0; font-size: 32px; background: var(--hero-slogan-gradient); -webkit-background-clip: text; background-clip: text; color: transparent; }
...
.turn-result { background: var(--app-surface-1); border: 1px solid var(--app-border-soft); border-radius: 16px; padding: 12px; box-shadow: 0 6px 16px var(--app-shadow-soft); }
.bubble-assistant { width: 100%; background: transparent; }
.streaming-tip { border: 1px solid var(--app-accent-border); border-radius: 12px; padding: 10px 12px; position: relative; overflow: hidden; background: var(--app-info-bg); box-shadow: inset 0 0 0 1px var(--app-info-border); }
.streaming-tip-core { display: inline-flex; align-items: center; gap: 8px; position: relative; z-index: 1; }
.streaming-ping { width: 9px; height: 9px; border-radius: 999px; background: var(--app-info-dot); box-shadow: 0 0 10px var(--app-info-shadow); animation: streaming-pulse 1.25s ease-in-out infinite; }
.streaming-label { font-size: 12px; letter-spacing: .08em; color: var(--app-accent-text); }
.streaming-dots { display: inline-flex; gap: 4px; }
.streaming-dots i { width: 4px; height: 4px; border-radius: 999px; background: var(--app-primary); animation: streaming-dot-wave 1s ease-in-out infinite; }
.streaming-dots i:nth-child(2){animation-delay:120ms;} .streaming-dots i:nth-child(3){animation-delay:240ms;}
.streaming-scanline { position: absolute; inset: 0; background: linear-gradient(115deg, transparent 0%, var(--app-scanline) 48%, transparent 100%); transform: translateX(-120%); animation: streaming-scan 2.2s linear infinite; }
...
.result-toolbar { margin-top: 10px; padding-top: 10px; border-top: 1px solid var(--app-surface-2); }
.view-output-btn, .output-close { height: 28px; border-radius: 8px; border: 1px solid var(--app-accent-border); background: var(--app-accent-bg); color: var(--app-accent-text); padding: 0 10px; cursor: pointer; }
.composer { padding-top: 6px; }
.composer-sidebar { margin-top: auto; position: relative; }
.composer-inner { width: 100%; border-radius: 12px; border: 1px solid var(--app-border-soft); background: var(--app-surface-1); display: flex; align-items: center; padding: 8px; }
.output-overlay { position: absolute; inset: 0; border-radius: 14px; border: 1px solid var(--app-border-soft); background: var(--app-surface-1); box-shadow: 0 12px 28px var(--app-shadow-soft); padding: 12px; display: flex; flex-direction: column; gap: 10px; z-index: 20; }
.output-overlay-header { display: flex; align-items: center; justify-content: space-between; font-size: 12px; color: var(--app-accent-text); }
.output-card { border: 1px solid var(--app-border-soft); background: var(--app-surface-hover); border-radius: 12px; padding: 10px; flex: 1; min-height: 0; display: flex; flex-direction: column; }
.output-card h4 { margin: 0 0 8px; color: var(--app-accent-text); font-size: 12px; }
.output-card-content { margin: 0; flex: 1; min-height: 0; overflow: auto; white-space: pre-wrap; word-break: break-word; color: var(--app-success); font-size: 12px; line-height: 1.6; }
.raw-line { margin: 0 0 8px; display: flex; gap: 8px; }
.raw-index { color: var(--app-muted-text); }
.raw-text-data-model { color: var(--app-warning); }
...
.content, .sidebar { scrollbar-width: thin; scrollbar-color: var(--app-muted-text) transparent; }
.content::-webkit-scrollbar, .sidebar::-webkit-scrollbar { width: 6px; }
.content::-webkit-scrollbar-thumb, .sidebar::-webkit-scrollbar-thumb { background: var(--app-muted-text); border-radius: 999px; }
.composer-inner input { flex: 1; border: none; outline: none; background: transparent; color: var(--app-text); padding: 10px 12px; font-family: 'JetBrains Mono', monospace; }
.composer-inner input::placeholder { color: var(--app-muted-text); }
.mode-select { height: 34px; border-radius: 8px; border: 1px solid var(--app-border-soft); background: var(--app-surface-1); color: var(--base-content-color); margin-right: 6px; }
.composer-inner button { height: 34px; min-width: 72px; border: 1px solid var(--app-accent-border); border-radius: 10px; background: var(--app-accent-bg); color: var(--app-accent-text); cursor: pointer; }
.sending { display: inline-flex; align-items: center; gap: 6px; }
.sending-dot { width: 8px; height: 8px; border-radius: 999px; background: var(--app-info-dot); animation: sending-pulse 1s ease-in-out infinite; }
